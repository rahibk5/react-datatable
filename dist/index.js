...
function App({ columns, data, filterOptions, searchable, customClass }) {
    ...
    const renderDropdowns = React.useMemo(() => {
        return processedFilterOptions === null || processedFilterOptions === void 0 ? void 0 : processedFilterOptions.map((filter) => (filter.searchable ? (jsxRuntimeExports.jsx("div", { className: "f-search-container", children: jsxRuntimeExports.jsx(react.Autocomplete, { label: "", classNames: {
                    base: "w-max bg-white dark:bg-[#122031]",
                    listboxWrapper: "max-h-[320px] bg-white dark:bg-[#122031] rounded shadow",
                    selectorButton: "text-default-500"
                }, placeholder: filter.name, className: "rounded px-3 py-2 bg-white dark:bg-[#122031]", onSelectionChange: (selection) => updateFilter(filter.uid, selection), children: filteredOptions(filter).map((option) => (jsxRuntimeExports.jsx(react.AutocompleteItem, { value: option.uid, className: "bg-white dark:bg-[#122031]", children: option.name }, option.uid))) }, filter.uid) })) :
