...
function App({ columns, data, filterOptions, searchable, customClass }) {
    ...
    const renderDropdowns = React.useMemo(() => {
        return processedFilterOptions === null || processedFilterOptions === void 0 ? void 0 : processedFilterOptions.map((filter) => (filter.searchable ? (jsxRuntimeExports.jsx("div", { className: "f-search-container", children: jsxRuntimeExports.jsx(Autocomplete, { label: "", placeholder: filter.name, onSelectionChange: (selection) => updateFilter(filter.uid, selection), children: filteredOptions(filter).map((option) => (jsxRuntimeExports.jsx(AutocompleteItem, { value: option.uid, children: option.name }, option.uid))) }, filter.uid) })) :
            (jsxRuntimeExports.jsx("div", { className: "f-dropdown-container", children: jsxRuntimeExports.jsxs(Dropdown, { children: [jsxRuntimeExports.jsx(DropdownTrigger, { className: "hidden sm:flex", children: jsxRuntimeExports.jsx(Button, { endContent: jsxRuntimeExports.jsx(ChevronDownIcon, { className: "text-small" }), size: "sm", variant: "flat", className: "min-w-min p-[20px]", children: filter.name }) }), jsxRuntimeExports.jsx(DropdownMenu, { disallowEmptySelection: false, "aria-label": "Table Columns", closeOnSelect: false, selectedKeys: filters[filter.uid] || new Set(), selectionMode: (filter === null || filter === void 0 ? void 0 : filter.selectMode) || "single", onSelectionChange: (selection) => updateFilter(filter.uid, selection), className: "max-h-64 overflow-y-auto", children: filteredOptions(filter).map((option) => (jsxRuntimeExports.jsx(DropdownItem, { className: "capitalize", children: capitalize(option.name) }, option.uid))) })] }, filter.uid) }))));
